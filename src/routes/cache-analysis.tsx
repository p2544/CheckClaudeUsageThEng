...
function CacheAnalysisPage() {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-3xl">{t('Cache Analysis')}</h2>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: 'var(--color-muted-foreground)' }}>
            {t('Prompt caching efficiency and savings')} ({getPeriodLabel(data.days, t)})
          </p>
        </div>
        <PeriodFilter value={period} onChange={setPeriod} />
      </div>
      ...
      {/* KPI cards */}
      <div className="grid grid-cols-4 gap-4">
        <KpiCard label={t('Cache Hit Rate')} value={formatPercent(overall.hitRate)} icon={<Zap size={18} style={{ color: 'var(--color-primary)' }} />} />
        <KpiCard label={t('Estimated Savings')} value={formatCost(overall.savings)} icon={<TrendingUp size={18} style={{ color: 'var(--color-muted-foreground)' }} />} />
        <KpiCard label={t('Cache Overhead')} value={formatCost(overall.overhead)} icon={<TrendingDown size={18} style={{ color: 'var(--color-muted-foreground)' }} />} />
        <KpiCard label={t('Net Savings')} value={formatCost(overall.netSavings)} icon={<DollarSign size={18} style={{ color: overall.netSavings >= 0 ? 'var(--color-primary)' : 'var(--color-destructive)' }} />} />
      </div>

      {/* ROI banner */}
      {overall.roi > 0 && (
        <div className="rounded-lg px-6 py-4 text-sm" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
          <span style={{ color: 'var(--color-muted-foreground)' }}>{t('Cache ROI:')} </span>
          <span className="font-medium" style={{ fontFamily: 'Georgia, serif', color: 'var(--color-foreground)' }}>
            {formatPercent(overall.roi)}
          </span>
          <span style={{ color: 'var(--color-muted-foreground)' }}> {t('— caching saved')} {formatPercent(overall.roi)} {t('of your total estimated cost')}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="rounded-lg p-6" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
          <h3 className="mb-4 text-lg">{t('Daily Cache Token Volume')}</h3>
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={dailyCacheTrend}>
              ...
              />
              <Area type="monotone" dataKey="cacheReadTokens" stackId="a" stroke="#c96442" fill="#c96442" fillOpacity={0.2} name={t('Cache Reads')} />
              <Area type="monotone" dataKey="cacheCreationTokens" stackId="a" stroke="#87867f" fill="#87867f" fillOpacity={0.15} name={t('Cache Writes')} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="rounded-lg p-6" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
          <h3 className="mb-4 text-lg">{t('Cache Efficiency by Model')}</h3>
          <div className="space-y-4">
            {modelCacheStats.map((m) => (
              <div key={m.model} className="space-y-1">
                <div className="flex items-center justify-between text-sm">
                  <span style={{ color: 'var(--color-foreground)' }}>{getModelDisplayName(m.model)}</span>
                  <span style={{ color: 'var(--color-muted-foreground)' }}>{formatPercent(m.hitRate)} {t('hit rate')}</span>
                </div>
                ...
                <div className="flex justify-between text-xs" style={{ color: 'var(--color-muted-foreground)' }}>
                  <span>{t('Savings:')} {formatCost(m.netSavings)}</span>
                  <span>{t('Cache reads:')} {formatTokens(m.cacheReadTokens)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-lg overflow-hidden" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
        <h3 className="px-6 py-4 text-lg" style={{ borderBottom: '1px solid var(--color-border)' }}>{t('Per-Project Cache Efficiency')}</h3>
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
              <th className="px-4 py-3 text-left font-medium" style={{ color: 'var(--color-muted-foreground)' }}>{t('Project')}</th>
              <th className="px-4 py-3 text-right font-medium" style={{ color: 'var(--color-muted-foreground)' }}>{t('Hit Rate')}</th>
              <th className="px-4 py-3 text-right font-medium" style={{ color: 'var(--color-muted-foreground)' }}>{t('Cache Reads')}</th>
              <th className="px-4 py-3 text-right font-medium" style={{ color: 'var(--color-muted-foreground)' }}>{t('Cache Writes')}</th>
              <th className="px-4 py-3 text-right font-medium" style={{ color: 'var(--color-muted-foreground)' }}>{t('Total Cost')}</th>
            </tr>
          </thead>
          <tbody>
            {projectCache.map((p) => (
